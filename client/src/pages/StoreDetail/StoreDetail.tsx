...
const featureIcons: Record<string, JSX.Element> = {
  ...
  "Air Conditioner": (<FontAwesomeIcon icon={faSnowflake} className="text-blue-500" />),
  "エアコン付き": (<FontAwesomeIcon icon={faSnowflake} className="text-blue-500" />),
  "Máy lạnh": <FontAwesomeIcon icon={faSnowflake} className="text-blue-500" />,
  ...
  "Thích hợp cho trẻ em": <FontAwesomeIcon icon={faChild} className="text-blue-500" />,
  "子供向け": <FontAwesomeIcon icon={faChild} className="text-blue-500" />,

  "Outdoor seating": (<FontAwesomeIcon icon={faChair} className="text-blue-500" />),
  "屋外席": <FontAwesomeIcon icon={faChair} className="text-blue-500" />,
  "Chỗ ngồi ngoài trời": ( <FontAwesomeIcon icon={faChair} className="text-blue-500" />),
  ...
  "Card payment": (<FontAwesomeIcon icon={faCreditCard} className="text-blue-500" />),
  "カード払い対応": (<FontAwesomeIcon icon={faCreditCard} className="text-blue-500" />),
  "Thanh toán bằng thẻ": (<FontAwesomeIcon icon={faCreditCard} className="text-blue-500" />),

  "Smoking room": (<FontAwesomeIcon icon={faSmoking} className="text-blue-500" />),
  "喫煙室": <FontAwesomeIcon icon={faSmoking} className="text-blue-500" />,
  "Phòng hút thuốc": ( <FontAwesomeIcon icon={faSmoking} className="text-blue-500" />  ),

  "Smoking: No": (<FontAwesomeIcon icon={faBan} className="text-blue-500" />),
  "Không hút thuốc": (<FontAwesomeIcon icon={faBan} className="text-blue-500" />),
  "禁煙": ( <FontAwesomeIcon icon={faBan} className="text-blue-500" />),

  "Free Parking": (<FontAwesomeIcon icon={faParking} className="text-blue-500" />),
  "Parking miễn phí": (<FontAwesomeIcon icon={faParking} className="text-blue-500" />),
  "無料駐車場'": ( <FontAwesomeIcon icon={faParking} className="text-blue-500" />),
};
...
function StoreDetail() {
  ...
  const style = data?.style.split("|").map((i) => i.trim());
  const description = data?.description.split("|").map((i) => i.trim());
  const address = data?.address.split("|").map((i) => i.trim());
    ...
    <div className="">
      ...
      <div className="flex px-16 py-8 space-x-8">
        ...
        <div className="w-2/3 space-y-8">
          ...
          {/* Location & Hours */}
          <div className="bg-white p-12 rounded shadow">
            <h2 className="text-3xl font-bold mb-4 p-6">
              {t("store.location_hour")}
            </h2>
              ...
              {Features &&
                Features.map((item, index) => (
                  <div key={index} className="flex items-center space-x-2">
                    {featureIcons[
                      item.features_name.split("|").map((i) => i.trim())[
                        language
                      ]
                    ] || (
                      <FontAwesomeIcon
          ...
          <div className="bg-white p-12 rounded shadow">
            ...
            {token ? (
              ...
              <p className="text-gray-600">
                {t("store.please")}
              </p> // Thông báo nếu chưa đăng nhập
            )}
            {/* Tổng quan đánh giá */}
            <RatingOverview
            ...
            {Reviews.length > 0 ? (
              ...
              <p className="text-black"> {t("store.no")}</p>
            )}
          </div>
        </div>
        {/* right */}
        ...
        <div className="w-1/3 h-full flex flex-col space-y-8 bg-[#F6F7F1] sticky top-32 mb-6 gap-4 p-12 ">
          ...
          <div className="bg-white p-12">
            <p className="flex items-center space-x-2">
              <span> {description && description[language]}</span>
            </p>
          </div>
          {/* Contact Info */}
          <div className="bg-white p-12">
          ...
          <div className="bg-white p-12">
            <p className="flex items-center space-x-2 mt-2">
              ...
              <span>{address && address[language]}</span>
            </p>
          </div>
        </div>
      </div>
